...
exports.composeErrorHandler = exports.composeGeneralHandler = exports.composeHandler = exports.findElysiaMeta = exports.isFnUse = exports.hasReturn = void 0;
const fast_querystring_1 = require("fast-querystring");
const handler_1 = require("./handler");
const utils_1 = require("./utils");
const error_1 = require("./error");
const ASYNC_FN = 'AsyncFunction';
const isAsync = (x) => x.constructor.name === ASYNC_FN;
...
const composeHandler = ({ method, hooks, validator, handler, handleError, meta, onRequest, config }) => {
    ...
    if (hooks?.afterHandle.length) {
        ...
        if (validator.response) {
            ...
            const handled = handler.constructor.name === ASYNC_FN
                ? 'await handler(c) '
                : 'handler(c)';
            if (hasSet)
    ...
    return createHandler({
        ...
        meta,
        SCHEMA: meta ? utils_1.SCHEMA : undefined,
        DEFS: meta ? utils_1.DEFS : undefined
    });
};
...
const composeErrorHandler = (app) => {
	...
	return ${app.event.error.find((fn) => fn.constructor.name === ASYNC_FN)
        ? 'async'
        : ''} function(request, error, set) {`;
    for (let i = 0; i < app.event.error.length; i++) {
        const handler = app.event.error[i];
        const response = `${handler.constructor.name === ASYNC_FN ? 'await ' : ''}onError[${i}]({
			request,
			code: error.code ?? 'UNKNOWN',
			error,
			set
